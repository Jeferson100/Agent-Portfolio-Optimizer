...
class TratatandoDadosFundamentalistas:
    ...
    async def completando_datas_faltantes(self) -> pd.DataFrame:
        ...
        dates_faltantes = pd.date_range(start=proxima_data, end=data_fim_dt, freq="QE")

        df_faltantes = pd.DataFrame({"datas": dates_faltantes})

        resultado = pd.concat([df, df_faltantes], ignore_index=True).sort_values(
            "datas"
        )
    ...
    async def deslocar_dados(self) -> pd.DataFrame:
        ...
        dados_deslocados[colunas_para_deslocar] = dados[colunas_para_deslocar].shift(
            self.periodos_deslocados
        )
